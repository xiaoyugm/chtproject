...
public:
	...
	struct XF : public Record
	{
		...
		char alignment_;	// 0x08: 1 = Text is wrapped at right border
		char rotation_;
...
public:
	...
	struct ColInfo : public Record
	{
		...
		virtual size_t Read(const char* data);
		virtual size_t Write(char* data);
		short firstColumnIndex_;
		short lastColumnIndex_;
		short columnWidth_;
		short XFRecordIndex_;
		short options_;
		short unused_;
...
public: // Cell functions
	...
	void SetColWidth(const size_t colindex , const short colwidth);
	short GetColWidth(const size_t colindex);
...
public:
	...
	void SetMergedRows(short mergedRows)		{mergedRows_ = mergedRows;} //assert(mergedRows>=2);
