...
{
//	CListCtrl &CList =  GetListCtrl();//»ñÈ¡µ±Ç°ÁÐ±í¿Ø¼þµÄÖ¸Õë

       CMenu       menu ,* pSubMenu;//¶¨ÒåÏÂÃæÒªÓÃµ½µÄcmenu¶ÔÏó

       menu.LoadMenu(IDC_POPLISTCONTROL);//×°ÔØ×Ô¶¨ÒåµÄÓÒ¼ü²Ëµ¥
...
//       int istat;//=CList.GetSelectionMark();//ÓÃistat´æ·Åµ±Ç°Ñ¡¶¨µÄÊÇµÚ¼¸Ïî

//    CString pString; //=CList.GetItemText(istat,0);//»ñÈ¡µ±Ç°ÏîÖÐµÄÊý¾Ý£¬0´ú±íÊÇµÚ0ÁÐ
//	pString="ÄúÑ¡ÔñµÄÂ·¾¶ÊÇ:"+pString ;//ÏÔÊ¾µ±Ç°Ñ¡ÔñÏî

//       MessageBox(pString);//ÏÔÊ¾µ±Ç°Ñ¡ÖÐµÄÂ·¾¶
       

	// Will return zero if no selection was made (TPM_RETURNCMD)
//	int nResult = menu.TrackPopupMenu(TPM_LEFTALIGN | TPM_RETURNCMD, point.x, point.y, this, 0);
	int nResult = pSubMenu->TrackPopupMenu (TPM_LEFTALIGN, oPoint.x, oPoint.y, this); //ÔÚÖ¸¶¨Î»ÖÃÏÔÊ¾µ¯³ö²Ëµ¥
	switch(nResult)
	{
		case 0: 
			break;
