...
void CFlatTabViewView::OnDraw(CDC* pDC)
{
//	其实标题栏由两部分组成  
//  同时更改可以在框架类中直接使用SetWindowText("NewName");
//	AfxGetMainWnd()->SetWindowText("煤矿安全监测");
	CFlatTabViewDoc* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
...
//	m_sheet1.SetBackColor(xtAfxData.clrWindow);
//	m_sheet2.SetBackColor(xtAfxData.clrWindow);
//	m_sheet3.SetBackColor(xtAfxData.clrWindow);
//	m_sheet4.SetBackColor(xtAfxData.clrWindow);
//	m_sheet5.SetBackColor(xtAfxData.clrWindow);
//	m_sheet6.SetBackColor(xtAfxData.clrWindow);
...
//	m_pFlatTabCtrl->InsertItem(5, _T("SQL Debugging"),   &m_sheet6);
	m_pFlatTabCtrl->InsertItem(0, _T("全部测点"),       m_pSheet);
...
*/	
	...
	m_pSheet = (CSampleFormView*)RUNTIME_CLASS(CSampleFormView)->CreateObject();
	if (!m_pSheet  || !((CWnd *)m_pSheet)->Create(
...
///sheet3.AddString(_T("Find in Files 1 Tab Output..."));
	...
	m_sheet4.SetFont(&xtAfxData.font);
	m_sheet5.SetFont(&xtAfxData.font);
	m_sheet6.SetFont(&xtAfxData.font);
*/
	return TRUE;
}
