...
BOOL CListCtrlDlg::OnInitDialog()
{
	...
	SetResize(IDC_BTN_UPDATE,        SZ_BOTTOM_LEFT, SZ_BOTTOM_LEFT);
	SetResize(IDC_BTN_RESET,         SZ_BOTTOM_LEFT, SZ_BOTTOM_LEFT);
...
void CListCtrlDlg::OnSelendokComboThemes()
{
	...
	case 2:
		m_bHotTracking = TRUE;
		m_bWinTheme = TRUE;
		m_header.SetTheme(new CXTHeaderCtrlThemeExplorer());
		CXTPPaintManager::SetTheme(xtpThemeOffice2000);
		break;
	case 3:
		m_bHotTracking = TRUE;
		m_bWinTheme = TRUE;
		m_header.SetTheme(new CXTHeaderCtrlTheme());
		CXTPPaintManager::SetTheme(xtpThemeOffice2000);
		break;
	}
