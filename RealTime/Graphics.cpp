...
void CGraphics::RecalcRects(RECT& rt)
{
	m_ClientRect = rt;
	SetPixelRect(rt);
	m_bM = (m_Rect.bottom - m_Rect.top) / 8;            //6
	m_tM = (m_Rect.bottom - m_Rect.top) / 8;          //15
	m_lM  = (m_Rect.right - m_Rect.left) / 13;         //18
	m_rM = (m_Rect.right - m_Rect.left) / 20;          //18
	...
	m_PlotRect.top		= m_Rect.top + m_tM;         //一起放缩
	m_PlotRect.bottom	= m_Rect.bottom - m_bM;

	GL = m_PlotRect.left;
	GR = m_PlotRect.right;
	GT = m_PlotRect.top - 0;    //22
	GB = m_PlotRect.bottom - 0;
	PX = GR - GL;
	PY = GB - GT;
...
/*	if (m_bPrinting || !m_bMemoryDraw)
	...
	else
	{
		...
		m_Bitmap = ::CreateCompatibleBitmap(hDC, m_Rect.right - m_Rect.left, 
...
/*	::BitBlt(hDC, m_Rect.left, m_Rect.top, m_Rect.right - m_Rect.left,
			 m_Rect.bottom - m_Rect.top, m_hDC, m_Rect.left, m_Rect.top, SRCCOPY);
...
void CGraphics::SetPixelRect(RECT rt)
{
	...
	m_Rect.top	  = (LONG)(rt.top - m_Ratio.ymin * Height + 22);
	m_Rect.right  = (LONG)(rt.right - (1 - m_Ratio.xmax) * Width);
	m_Rect.bottom = (LONG)(rt.bottom - (1 - m_Ratio.ymax) * Height +20);
}
...
void CGraphics::XAxisTitle(const char* Title, int Pos)
{
	...
	if (m_Font)
	{
		...
		HFONT hOldFont = (HFONT)::SelectObject(m_hDC, m_Font);
		::SetTextColor(m_hDC, RGB(0, 0, 0)); 
///		::SetTextColor(m_hDC, m_nTitleColor);   TOP
		SetStringAlign(CENTER, CENTER);
		if (Pos == TOP)
			PrintString( GR - 2* m_rM, m_Rect.bottom -m_tM/2, 0, Title);
		else  if(Pos == BOTTOM)
			PrintString((GL + GR) / 2,  GT - m_bM/6 , 0, Title); //GB
		::SelectObject(m_hDC, hOldFont);
		::DeleteObject(m_Font);
		::SetBkMode(m_hDC, bm);
	}
...
void CGraphics::YAxisTitle(const char* Title, int Pos)
{
	...
	if (Pos == LEFT)
	{
		m_LogFont.lfOrientation= -900;       //900
		m_LogFont.lfEscapement = -900;       //900
	}
	else
	{
...
void CGraphics::Grid()
{
	...
	for(i = 1; i < XGridTicks; i ++)
	{
		...
		DrawLine(j, GB + 1, j, GB - 1);
	}

	for(i = 1; i < YGridTicks; i ++) 
	{
		j = GB + (int)(1.0 * i * (GT-GB) / YGridTicks);
...
void CGraphics::XAxis()
{
	...
	for(i = 0; i <= XTicks; i ++) 
	{
		...
		if((i % j) == 0)
		{
			...
			PrintString(xb, GT + m_bM / 5 -5, 0, str);
		}
		else 
			ye = GT + m_bM / 14;
		if (i > 0 && i < XTicks)
...
void CGraphics::YAxis()
{
	...
	for(i = 0; i <= YTicks; i ++)     
	{
		yb = ye = (int)(GT + 1.0 * PY * i / YTicks );    //GTT
		if((i % j) == 0) 
		{
			xb = GL - m_lM / 10;
...
//				value = (float)(m_Scale.ymin - i * (m_Scale.ymin - m_Scale.ymax) / YTicks);
//				Format(m_nYDecimal, str, value);
				PrintString(GR + m_rM / 12, yb+15, 0, str);
///				PrintString(GR + m_rM / 6, yb+15, 0, str);
			}
			else
				xe = GR + m_rM / 20;
			if (i > 0 && i < YTicks)
...
{
	HPEN m_CurPen = ::CreatePen(PS_SOLID, 0, RGB(128, 128, 128));
	HPEN m_OldPen = (HPEN)::SelectObject(m_hDC, m_CurPen);
	HBRUSH hBrush = ::CreateSolidBrush(RGB(128, 128, 128));
	HBRUSH hBrold = (HBRUSH)::SelectObject(m_hDC, hBrush);
	int w  = (m_Rect.right - GR) / 20 + 1;
	int xb = GR + 2 * n;
	::Rectangle(m_hDC, GR + (int)(6.2 * w), GT - m_LogFont.lfHeight, 
				m_Rect.right - (int)(0.8 * w), GT - 3 * (n + 0.6) * m_LogFont.lfHeight);
...
///				GT - 2 * (n + 1) * m_LogFont.lfHeight + m_LogFont.lfHeight);
	::Rectangle(m_hDC, GR + 4*w, GT, m_Rect.right - 2 * w, 
				GT - 3 * (n + 0.8) * m_LogFont.lfHeight + m_LogFont.lfHeight);
	::SelectObject(m_hDC, hBrold);
	::DeleteObject(hBrush);
}
