...
CSerialPort::~CSerialPort()
{
	...
	// if the port is still opened: close it ,added by gjw
	if (m_hComm != NULL)
	{
		CloseHandle(m_hComm);
		m_hComm = NULL;
	}
	// Close Handles   added by gjw
...
{
	//sert(portnr > 0 && portnr < 5);  //这样设置可限制串口号为1，2，3，4
...
//else// ELSE, it hasn't been created yet 
...
*/
   ...
   //set DTR
	BOOL bsetdtr=0;
	bsetdtr=EscapeCommFunction(m_hComm,SETDTR);
...
//				BOOL bsetdtr=0;
//				bsetdtr=EscapeCommFunction(port->m_hComm,SETDTR);
				//if(bsetdtr) 
				//	AfxMessageBox("SetDTR success");
				//else
				//	AfxMessageBox("SetDTR fail");
				...
				//BOOL bsetdtr=0;
			//	bsetdtr=EscapeCommFunction(port->m_hComm,CLRDTR);
